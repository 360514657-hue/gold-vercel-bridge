...
test('macro numeric signs, critical missing neutral and future source exclusion',()=>{const f=fixture();assert.equal(macroV2(f.input.macro_facts,now).score,100);f.input.macro_facts.DXY.available_at=now+1;const x=macroV2(f.input.macro_facts,now);assert.equal(x.bias,'NEUTRAL');assert.equal(x.acceptable,false);});
...
test('changing macro permission never changes structural entry or stop',()=>{const a=fixture('BUY','BULLISH'),b=fixture('BUY','BEARISH');const x=decide(a).contract,y=decide(b).contract;assert.equal(x.entry,y.entry);assert.equal(x.stop_loss,y.stop_loss);assert.equal(x.take_profit,y.take_profit);});
test('macro cannot manufacture entry from missing retest or TP without levels',()=>{const f=fixture();f.state.candidate.retest=null;assert.equal(decide(f).contract.entry,null);const g=fixture();g.result.key_levels=[];assert.equal(decide(g).contract.take_profit,null);});
