...
export function watchHandler({verify=signedRequest,run=()=>signal({requireDurable:true}),logger=x=>console.log(JSON.stringify(x))}={}){return async(req,res)=>{
 ...
 try{const x=await run();return res.status(200).json({ok:true,stage:x.stage,action:x.action,signal_id:x.signal_id,time:x.time,state_version:x.debug?.persistence?.state_version??null,updated_at:x.debug?.persistence?.updated_at??null,signal_only:true,order_executed:false});}
