...
export async function signedRequest(req,env=process.env){
 ...
 const signature=req.headers['upstash-signature'];if(typeof signature!=='string')return false;
 ...
 try{return await new Receiver({currentSigningKey,nextSigningKey,devMode:false}).verify({signature,body:Buffer.concat(chunks).toString('utf8'),url,clockTolerance:0});}catch{return false;}
}
export function watchHandler({verify=signedRequest,run=()=>signal({requireDurable:true}),logger=x=>console.log(JSON.stringify(x))}={}){return async(req,res)=>{
 ...
 try{if(!await verify(req))return res.status(401).json({error:'INVALID_QSTASH_SIGNATURE'});}catch{return res.status(503).json({action:'NO_TRADE',stage:'DEGRADED',error:'WATCH_CONFIG_REQUIRED'});}
 try{const x=await run();return res.status(200).json({ok:true,email_status:x.email_status??'NOT_CONFIGURED',execution:x.execution,stage:x.stage,action:x.action,signal_id:x.signal_id,time:x.time,state_version:x.debug?.persistence?.state_version??null,updated_at:x.debug?.persistence?.updated_at??null,signal_only:true,order_executed:false});}
