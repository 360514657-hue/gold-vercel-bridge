...
export async function signedRequest(req,env=process.env){
 const currentSigningKey=env.QSTASH_CURRENT_SIGNING_KEY,nextSigningKey=env.QSTASH_NEXT_SIGNING_KEY,url=env.WATCH_DESTINATION_URL;
...
export function watchHandler({verify=signedRequest,run=()=>signal({requireDurable:true}),logger=x=>console.log(JSON.stringify(x))}={}){return async(req,res)=>{
 ...
 try{const x=await run();return res.status(200).json({ok:true,stage:x.stage,action:x.action,signal_id:x.signal_id,time:x.time,signal_only:true,order_executed:false});}
