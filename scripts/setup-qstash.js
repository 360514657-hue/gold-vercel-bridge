...
if(!env.QSTASH_TOKEN||!env.QSTASH_CURRENT_SIGNING_KEY||!env.QSTASH_NEXT_SIGNING_KEY||!env.WATCH_DESTINATION_URL){console.log('NOT_CONFIGURED: QStash credentials and WATCH_DESTINATION_URL required');process.exit(2);}
...
const url=new URL(env.WATCH_DESTINATION_URL);
if(url.protocol!=='https:'||url.pathname!=='/api/watch/xauusd'||url.search||url.username||url.password){console.log('REFUSED: invalid watcher destination');process.exit(2);}
try{
