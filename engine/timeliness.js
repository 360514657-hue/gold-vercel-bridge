...
export function timing(state,result,input){
 ...
 if(c?.timing_missed&&!['INVALIDATED','TP1','TP2'].includes(c.stage)){result.stage='MISSED';result.action='NO_TRADE';result.rating='C';result.reason.push('ENTRY_MISSED_NO_CHASE');return;}
 if(result.action==='ALLOW_ORDER'||c?.stage==='ACTIVE'){result.stage='SIGNAL_READY';return;}
...
export function missed(c,price){if(!c?.retest||['INVALIDATED','TP1','TP2'].includes(c.stage))return false;const reference=c.retest.entry_reference??c.trigger;const edge=c.direction==='BUY'?Math.max(reference,c.retest.close):Math.min(reference,c.retest.close);return (c.direction==='BUY'?price-edge:edge-price)>chaseLimit();}
